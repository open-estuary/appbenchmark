...
if [ $# -lt 1 ] ; then 
    ...
    do
        #echo "${APP_ROOT}/apps/mysql/alisql/scripts/run_single_server.sh ${cur_inst} $@" >> ${tmp_mysql_init_file}
        ${APP_ROOT}/apps/mysql/alisql/scripts/run_single_server.sh ${cur_inst} ${1}
        let "cur_inst++"
    done
