...
if [ "$(tool_check_exists ${BUILD_DIR}/${TARGET_DIR}/${SUBBUILD_DIR}/sql/mysqld)"  == 0 ]; then
      echo "AliSQL has been built successfully"
      exit 0
fi
...
if [ x$(uname -m) == x"aarch64" ] ; then
    ...
    $(tool_add_sudo) cp ${APP_ROOT}/apps/mysql/alisql/bugfix/storage/innobase/include/sync0sync.ic \
...
echo "**********************************************************************************"
echo "Finish build alisql server successfully"
popd > /dev/null

##########################################################################################
echo "Build alisql successfully"
