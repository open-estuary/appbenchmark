...
if [ "$4" == "init" ] ; then
    ...
    ${APP_ROOT}/apps/mysql/common/scripts/readall.sh $1 $2 $3  ${default_table_count}
elif [ "$4" == "test" ] ; then
    inst_num=${5}
    thread_per_inst=1
    if [ ! -z "${6} " ] ; then
    ...
    ${APP_ROOT}/apps/mysql/alisql_2/scripts/sysbench_lots_instances.sh ${1} ${inst_num} ${default_table_size} ${default_table_count} ${thread_per_inst}
else 
