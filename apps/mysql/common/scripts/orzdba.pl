...
# 
my $MYSQL    = qq{mysql -s --skip-column-names -uroot -p123456 -P$port };
$MYSQL      .= qq{-S$socket } if defined $socket;
my $TCPRSTAT = "tcprstat --no-header -t 1 -n 0 -p $port";
