...
do
    ...
    do
        ...
        if [ ${cpunum} -eq 1 ] ; then
           nohup numactl --membind=0 --physcpubind=0 lat_mem_rd -P 1 ${flags} 32M 128 >> ~/result_memory/lat/result_${IP} 2>&1
        else
           nohup numactl --membind=0 --physcpubind=0-${cpuindex} lat_mem_rd -P ${cpunum} ${flags} 32M 128  >> ~/result_memory/lat/result_${IP} 2>&1
        fi
    done
done
...
do
    ...
    do
        ...
        let "cpuindex=cpuindex-1"
        if [ ${cpunum} -eq 1 ] ; then
           nohup numactl --membind=1 --physcpubind=0 lat_mem_rd -P 1 ${flags} 32M 128  >> ~/result_memory/lat/result_${IP} 2>&1
        else
           nohup numactl --membind=1 --physcpubind=0-${cpuindex} lat_mem_rd -P ${cpunum} ${flags} 32M 128  >> ~/result_memory/lat/result_${IP} 2>&1
        fi
    done
done 
